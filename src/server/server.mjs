...
log("process.env.HOST: ", process.env.HOST)
...
export default function App(){
  ...
  server.on('listening', function() {
    ...
    //console.log(`IP address on http://${localhost}:${PORT} <- PC IP`);
    //console.log(`IP address 1 on http://${HOST}:${PORT}`)
    //console.log(`IP address 2 on http://127.0.0.1:${PORT} `);
    log("")
    log(`Web Server on `,chalk.green(`http://localhost:${PORT}`),`<- Default DEV Test!`);
    log("")
    //console.log(`IP address 4 on http://localhost:${PORT}/ip <- DEV IP Test`);
